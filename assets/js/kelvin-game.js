(() => {
  ...
  function launchTextFireworks() {
    ...
    const lines=['No matter how far we are,','my heart still finds you.'];const fontSize=Math.max(22,Math.min(54,rect.width/14.2));
    ...
    bx.font=`700 ${fontSize}px Georgia, serif`;bx.textAlign='center';bx.textBaseline='middle';bx.fillStyle='#fff';
    lines.forEach((line,index)=>bx.fillText(line,buffer.width/2,fontSize*(1+index*1.25)));
    const pixels=bx.getImageData(0,0,buffer.width,buffer.height).data;const points=[];const step=rect.width<600?3:4;const textTop=Math.max(60,rect.height*.27);
    ...
    function animate(now){
      ...
      if(elapsed>3.25){const reveal=clamp((elapsed-3.25)/1.1,0,1);fx.save();fx.globalAlpha=.16*reveal;fx.shadowColor='#ffd9ed';fx.shadowBlur=18;fx.fillStyle='#fff4d6';fx.font=`700 ${fontSize}px Georgia, serif`;fx.textAlign='center';fx.textBaseline='middle';lines.forEach((line,index)=>fx.fillText(line,rect.width/2,textTop+fontSize*(1+index*1.25)));fx.restore()}
      ...
      for(const particle of particles){const local=clamp((elapsed-particle.delay)/2.65,0,1);if(local<=0)continue;const progress=ease(local);const spiral=(1-progress)*44;const x=particle.sx+(particle.tx-particle.sx)*progress+Math.cos(particle.angle+progress*9)*spiral;const y=particle.sy+(particle.ty-particle.sy)*progress+Math.sin(particle.angle+progress*9)*spiral*.55;const twinkle=local===1?.86+Math.sin(elapsed*2.2+particle.angle)*.14:1;const particleAlpha=Math.min(1,local*1.6)*twinkle;if(local<.94){const tail=18*(1-local)+5;const gradient=fx.createLinearGradient(x-tail,y-tail*.55,x,y);gradient.addColorStop(0,'transparent');gradient.addColorStop(1,`hsla(${particle.hue},100%,78%,${particleAlpha*.65})`);fx.strokeStyle=gradient;fx.lineWidth=1.2;fx.beginPath();fx.moveTo(x-tail,y-tail*.55);fx.lineTo(x,y);fx.stroke()}fx.shadowColor=`hsl(${particle.hue},100%,72%)`;fx.shadowBlur=local>.88?12:6;fx.fillStyle=`hsla(${particle.hue},100%,88%,${particleAlpha})`;fx.beginPath();fx.arc(x,y,local>.9?2.15:1.35,0,Math.PI*2);fx.fill()}
      fx.shadowBlur=0;fx.globalCompositeOperation='source-over';if(!surprise.hidden)requestAnimationFrame(animate)
    }
  ...
  const latestLetter=document.querySelector('#latest-letter');const latestLetterTitle=document.querySelector('#latest-letter-title');const latestLetterDate=document.querySelector('#latest-letter-date');const latestLetterBody=document.querySelector('#latest-letter-body');const latestLetterClose=document.querySelector('#latest-letter-close');
